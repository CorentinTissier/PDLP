...
def mps_parsing(mps):
    variable_names=mps.variable_names()
    constraints_dict=mps.constraints
    rhs_dict=mps.get_rhs()

    # Create a mapping from variable names to column indices
    var_to_col = {v: i for i, v in enumerate(variable_names)}

    key_E=[]
    key_L=[]
    key_U=[]
    for key in constraints_dict:
        if constraints_dict[key]['type']=='E':
            key_E.append(key)
        if constraints_dict[key]['type']=='L':
            key_L.append(key)
        if constraints_dict[key]['type']=='U':
            key_U.append(key)

    equality_constraint_dict={key: constraints_dict[key] for key in key_E}
    ineq_constraint_dict={key: constraints_dict[key] for key in key_L+key_U}
    
    #Generate A
    num_rows = len(key_E)
    num_cols = len(variable_names)
    A=sp.lil_matrix((num_rows, num_cols))
    b=np.zeros((num_rows,1))
    for i, (constraint_name, constraint_dict) in enumerate(equality_constraint_dict.items()):
        b[i]=rhs_dict[constraint_name]
        coefficients = constraint_dict['coefficients']
        for var_name, coeff in coefficients.items():
            j = var_to_col[var_name]
            A[i, j] = coeff
    A = A.tocsr()
    
    #Generate B
    num_rows = len(key_L+key_U)
    G=sp.lil_matrix((num_rows, num_cols))
    h=np.zeros((num_rows,1))
    for i, (constraint_name, constraint_dict) in enumerate(ineq_constraint_dict.items()):
        coefficients = constraint_dict['coefficients']
        constraint_type = constraint_dict['type']

        if constraint_type=='L':
            h[i]=-rhs_dict[constraint_name]
        else:
            h[i]=rhs_dict[constraint_name]
        for var_name, coeff in coefficients.items():
            j = var_to_col[var_name]
    ...
    l=np.array([mps.variables[list(mps.variables.keys())[0]][el]['lower'] for el in mps.variables[list(mps.variables.keys())[0]]]).reshape(-1,1)
    u=np.array([mps.variables[list(mps.variables.keys())[0]][el]['upper'] for el in mps.variables[list(mps.variables.keys())[0]]]).reshape(-1,1)
    c=np.array([mps.objectives[list(mps.objectives.keys())[0]]['coefficients'][el] for el in mps.objectives[list(mps.objectives.keys())[0]]['coefficients']]).reshape(-1,1)
...
def small_lp1():
    ...
    """
    l=np.array([0.0, 0.0, 0.0, 0.0]).reshape(-1,1)  # variable_lower_bound
    u=np.array([2.0, 4.0, 6.0, 3.0]).reshape(-1,1)  # variable_upper_bound

    c=np.array([5.0, 2.0, 1.0, 1.0]).reshape(-1,1)  # objective_vector
...
def small_lp2():
    ...
    G = np.array([[ -2,  1,-1],  # Red constraint left side
                [-1,  -3,-2]])  # Yellow constraint left side

    h = np.array([[-8],  # Red constraint right side
                [-18]])  # Yellow constraint right side

    A = np.array([[1, 2,1]])  # Green constraint left side
    b= np.array([[10]])       # Green constraint right side

    l=np.array([[0],[0],[0]])
    u=np.array([[float("inf")],[4],[5]])
    u=np.array([[10],[4],[5]])
